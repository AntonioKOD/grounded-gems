...
export async function POST(req: NextRequest, context: any) {
  ...
  try {
    ...
    normalizedInvitees[inviteeIdx].status = 'accepted'
    ...
    try {
      ...
      if (ownerId) {
        ...
        if (owner && owner.email) {
          await sendEmail({
            to: owner.email,
            subject: `Your journey invite was accepted!`,
            html: `<p>Hi ${owner.name || owner.email},</p><p>${user.name || user.email} has accepted your invite to the journey <b>${journey.title}</b>.</p>`
          })
        } else {
        ...
        await payload.create({
          ...
          data: {
            recipient: ownerId,
            type: 'reminder',
            title: `${user.name || user.email} accepted your Gem Journey invite`,
            message: `${user.name || user.email} has accepted your invite to the journey "${journey.title}"`,
      ...
      console.error('Error sending owner accept email or notification:', err)
    }
