  ...
  try {
    ...
    switch (type) {
      ...
      case 'locations':
        ...
        const locations = await searchLocationsAction(locationSearchParams as any)
      ...
      case 'events':
        ...
        const events = await searchEventsAction(eventSearchParams as any)
      ...
      default:
        ...
        // Search locations
        const allLocations = await searchLocationsAction(query)
        ...
        // Search events
        const allEvents = await searchEventsAction(query)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user && query.length >= 2) {
      try {
        const quickLocations = await searchLocationsAction(query)
