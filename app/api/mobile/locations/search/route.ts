...
// POST /api/mobile/locations/search - Search locations for mobile
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { query, coordinates, radius = 25 } = body
    ...
    try {
      ...
      if (authHeader?.startsWith('Bearer ')) {
        ...
        currentUser = user
        console.log('Mobile locations search: Authenticated user:', currentUser?.name)
      }
    } catch (authError) {
      console.log('Mobile locations search: No authenticated user or auth error:', authError)
    }
    ...
    // Format locations for mobile frontend
