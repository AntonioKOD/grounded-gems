...
interface CommentSystemDarkProps {
  postId: string
  user?: {
    id: string
    name: string
...
export function CommentSystemDark({ 
  postId, 
  user, 
  initialComments = [], 
  className,
  autoShow = false 
}: CommentSystemDarkProps) {
  ...
  // Comment component for dark theme
  const CommentItem = useCallback(({ comment, isReply = false }: { comment: Comment; isReply?: boolean }) => {
    ...
    return (
      ...
      >
        <div className={cn(
          "relative p-4 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg backdrop-blur-sm",
          sentiment.bgColor,
          ...
          <div className="absolute -top-2 -right-2">
            <div className={cn(
              "w-8 h-8 rounded-full flex items-center justify-center text-lg border-2 border-black/20 shadow-sm backdrop-blur-sm",
              sentiment.bgColor
            )}>
              {sentiment.emoji}
            </div>
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs px-2 py-0 border-white/20 bg-white/10 text-white/90">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 p-0 text-white/60 hover:text-white hover:bg-white/10"
                >
                  <MoreHorizontal className="h-3 w-3" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-32 bg-black/90 border-white/20">
                <DropdownMenuItem className="text-xs text-white/90 hover:text-white focus:text-white">Report</DropdownMenuItem>
                <DropdownMenuItem className="text-xs text-white/90 hover:text-white focus:text-white">Share</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className={cn(
                  ...
                  isCommentLiked ? "text-red-400 bg-red-500/20" : "text-white/60 hover:text-white/90 hover:bg-white/10"
                )}
              >
              ...
              {hasReplies && !isReply && (
                <Button
                  ...
                  className="h-7 px-2 text-xs text-white/60 hover:text-white/90 hover:bg-white/10 flex items-center gap-1"
                >
        ...
        {hasReplies && !isReply && isExpanded && (
          <div className="relative mt-2">
            {/* Connection line */}
            <div className="absolute left-6 top-0 w-px h-full bg-gradient-to-b from-white/30 to-transparent" />
    ...
    <div className={cn("w-full", className)}>
      ...
      {!autoShow && (
        <Button
          ...
          className="w-full justify-start p-4 h-auto hover:bg-white/5 transition-colors text-white"
        >
          <MessageCircle className="w-5 h-5 mr-3 text-white/60" />
      ...
      <AnimatePresence>
        {showComments && (
          <motion.div
            ...
            className={cn("border-t border-white/10", !autoShow && "border-t")}
          >
            <div className="p-4">
              {/* New comment form */}
              {user ? (
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <Textarea
                        ...
                        className="min-h-[80px] resize-none bg-white/5 border-white/20 text-white placeholder:text-white/60 focus:border-[#FF6B6B] focus:ring-[#FF6B6B]/20"
                      />
              ...
              <div className="space-y-1">
                {isLoadingComments ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                      <span className="text-sm text-white/60">Loading comments...</span>
                    </div>
                  </div>
                ) : (
                  <AnimatePresence>
