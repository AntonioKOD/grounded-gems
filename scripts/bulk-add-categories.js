...
async function addCategories() {
  console.log('🚀 Starting to add categories to the database...\n');
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    try {
      console.log(`${i + 1}/${categories.length} Adding: ${category.name}`);
      ...
      if (response.ok) {
        ...
        console.log(`✅ Created: ${category.name} (ID: ${result.doc?.id})`);
        successCount++;
      } else {
        const errorText = await response.text();
        console.log(`❌ Failed: ${category.name} - ${response.status}: ${errorText}`);
        errors.push(`${category.name}: ${response.status} - ${errorText}`);
        errorCount++;
      }
    } catch (error) {
...
fetch(`${PAYLOAD_URL}/api/categories`, { method: 'GET' })
  .then(response => {
    if (response.ok) {
      console.log('✅ Payload API is accessible. Starting category addition...\n');
      addCategories();
    } else {
      console.log('❌ Cannot access Payload API. Make sure your dev server is running on http://localhost:3000');
    }
  })
  .catch(error => {
    console.log('❌ Cannot connect to dev server. Make sure it\'s running on http://localhost:3000');
